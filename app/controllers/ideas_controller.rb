class IdeasController < ApplicationController
  ...
  # GET /ideas.xml
  def index
    @ideas = Idea.paginate(:per_page => 10, :page => params[:page])
  ...
  def tagged
    ...
    @ideas = Idea.find_tagged_with(params[:tag])
                                         
    @tags = [] # Idea.tag_counts :conditions => conditions
    
    if @ideas.empty?
      error("No posts found...", 404)
    else
      respond_to do |wants| 
        wants.html { render :action => 'index' }   
        wants.rss { render :action => 'index' }
